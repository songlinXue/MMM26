...
class FaceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not isinstance(video_name, str) or not video_name.strip():
            print(f"Warning: Invalid video_name at index {idx}: {video_name}")
            raise IndexError(f"Skipping invalid video_name at index {idx}")
        
        if '.ipynb_checkpoints' in video_name:
            print(f"Warning: Skipping Jupyter checkpoint file at index {idx}: {video_name}")
            raise IndexError(f"Skipping Jupyter checkpoint file at index {idx}") 
