...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, afan=None):
    ...
    def forward(self, x):
        ...
        if self.afan is not None:
            assert out.size(1) == self.afan.crossnorm.in_channels, \
                f"AFAN input channels ({self.afan.crossnorm.in_channels}) do not match output channels of conv2 ({out.size(1)})"
            out = self.afan(out)
...
class ResNet(AbstractResNet):
    ...
    def __init__(self, block, layers, num_classes=4, max_iter=-1, normalized=True, use_bias=False, simsiam=False):
        super(ResNet, self).__init__(block, layers, num_classes)
        feat_dim = 512 * block.expansion

        if normalized:
            self.fc0 = NormedLogisticRegression(feat_dim, 1, use_bias=use_bias)
            self.fc1 = NormedLogisticRegression(feat_dim, 1, use_bias=use_bias)
            self.fc2 = NormedLogisticRegression(feat_dim, 1, use_bias=use_bias)
            self.fc3 = NormedLogisticRegression(feat_dim, 1, use_bias=use_bias)
        else:
            self.fc0 = LogisticRegression(feat_dim, 1)
            self.fc1 = LogisticRegression(feat_dim, 1)
            self.fc2 = LogisticRegression(feat_dim, 1)
            self.fc3 = LogisticRegression(feat_dim, 1)

        self.bn_scale = nn.BatchNorm1d(1)
        self.fc_scale = nn.Linear(feat_dim, 1)
        ...
        if simsiam:
            self.projection = nn.Sequential(
                nn.Linear(feat_dim // block.expansion, feat_dim // block.expansion, bias=False),
                nn.BatchNorm1d(feat_dim // block.expansion),
                nn.ReLU(inplace=True), # first layer
                nn.Linear(feat_dim // block.expansion, feat_dim // block.expansion, bias=False),
                nn.BatchNorm1d(feat_dim // block.expansion),
                nn.ReLU(inplace=True), # second layer
                nn.Linear(feat_dim // block.expansion, feat_dim // block.expansion, bias=False),
                nn.BatchNorm1d(feat_dim // block.expansion, affine=False) # output layer
            )

            proj_feat_dim = 128
            self.predictor = nn.Sequential(
                nn.Linear(feat_dim // block.expansion, proj_feat_dim, bias=False),
                nn.BatchNorm1d(proj_feat_dim),
                nn.ReLU(inplace=True), # hidden layer
                nn.Linear(proj_feat_dim, feat_dim // block.expansion)
            )
    ...
    def snapshot_weight(self):
        ...
        self.w3_old = self.fc3.weight.data.clone()

    def update_weight_v5(self, alpha=0.99):
        ...
        beta3 = normalizer(self.fc3.weight.data.squeeze())

        # 找到最近邻
        beta0_fr = min(
            ((beta0 * beta1).sum().item(), beta1),
            ((beta0 * beta2).sum().item(), beta2),
            ((beta0 * beta3).sum().item(), beta3),
            key=lambda x: x[0]
        )[1]

        beta1_fr = min(
            ((beta1 * beta0).sum().item(), beta0),
            ((beta1 * beta2).sum().item(), beta2),
            ((beta1 * beta3).sum().item(), beta3),
            key=lambda x: x[0]
        )[1]

        beta2_fr = min(
            ((beta2 * beta0).sum().item(), beta0),
            ((beta2 * beta1).sum().item(), beta1),
            ((beta2 * beta3).sum().item(), beta3),
            key=lambda x: x[0]
        )[1]

        beta3_fr = min(
            ((beta3 * beta0).sum().item(), beta0),
            ((beta3 * beta1).sum().item(), beta1),
            ((beta3 * beta2).sum().item(), beta2),
            key=lambda x: x[0]
        )[1]
        ...
        self.fc3.weight.data = (normalizer(alpha * beta3 + (1 - alpha) * beta3_fr) * torch.norm(self.fc3.weight.data)).unsqueeze(0)

        return ((beta0 * beta1).sum() + (beta0 * beta2).sum() + (beta0 * beta3).sum() +
                (beta1 * beta2).sum() + (beta1 * beta3).sum() + (beta2 * beta3).sum()) / 6
    ...
    def forward(self, x, fc_params=None, out_type='ce_sep', scale=None):
        ...
        if out_type == 'ce_sep':
            ...
            logits = (self.fc0(feat, scale) + self.fc1(feat, scale) +
                      self.fc2(feat, scale) + self.fc3(feat, scale)) / 4
            return feat, feat, logits
        else:
            raise RuntimeError("Unsupported out_type")
